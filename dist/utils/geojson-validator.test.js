...
const geojson_validator_1 = require("./geojson-validator");
...
describe('Tests for validateGeometry function', () => {
    test('Validates valid point geometry', () => {
        ...
        expect(geojson_validator_1.validateGeometry(geometry)).toBe(undefined);
    });
    ...
    test('Throws error when passed full GeoJSON rather than just the geometry', () => {
        ...
        expect(() => {
            geojson_validator_1.validateGeometry(geojson);
        }).toThrowError(InvalidGeometry_1.InvalidGeometry);
    });
    ...
    test('Throws error when polygon does not follow right-hand rule', () => {
        ...
        expect(() => {
            geojson_validator_1.validateGeometry(geometry);
        }).toThrowError(InvalidGeometry_1.InvalidGeometry);
        expect(() => {
            geojson_validator_1.validateGeometry(geometry);
...
//# sourceMappingURL=geojson-validator.test.js.map